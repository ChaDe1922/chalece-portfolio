...
/** Shared, data-driven graded assessment for lab lessons. Supports `mc-code`,
 *  `mc-text`, and `fill` questions plus a self-checked reflection rubric. Pass a
 *  lesson's quiz data; the component marks the slide complete once every check
 *  is solved. The recursion lab keeps its own copy; new lessons use this one. */
...
export type QuizData = {
  ...
  readonly reflection: {
    ...
    readonly rubric: readonly { readonly level: string; readonly criteria: string }[];
  };
};
          ...
          <div className="space-y-2">
            {q.labels.map((label, i) => (
              <div key={label} className="flex items-center gap-3">
                <label htmlFor={`q-${q.id}-fill-${i}`} className="w-36 text-sm text-muted-foreground">
                  {label}
                </label>
                <input
                  id={`q-${q.id}-fill-${i}`}
...
/** Graded quick checks (each with rationale and a per-objective success cover),
 *  then a synthesis reflection. Marks the slide complete when all checks pass. */
