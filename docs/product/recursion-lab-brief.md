...
**Shipped (2026-06-05):** `/lab/recursion` now delivers one complete, taught lesson adapted from `curriculum-dev-agent/courses/recursion-intro` (M1 L1). Nine slides with graceful staggered content reveals (each block rises in via the brand `enter-up`, reduced-motion safe), opening on a cover that states the lesson and its learning objectives with a Begin button. The mirror hook is a 3D infinity-mirror tunnel (React Three Fiber, code-split to the lab route only, with the CSS illusion as the reduced-motion / no-WebGL fallback). Teaching every concept before it is assessed: cover (objectives) to mirror room (experience) to nested dolls (base case, recursive case, plus the plain definition of recursion) to what it is and where you will meet it (definition + why + applications gallery) to how to write one in Python (the recipe + build countdown + predict the output) to the call stack (LIFO framing + countdown visualizer) to no base case = RecursionError (run the broken function, watch it crash, reframed as protection, plus the one-line fix) to the fractal tree (payoff) to a graded assessment (multiple choice with per-option feedback, a fill-in trace, and a synthesis reflection with a self-assessment rubric). The §4.2 manipulatives arc below is archived. The SlideDeck engine is unchanged and still powers it.
